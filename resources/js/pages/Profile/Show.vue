        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div class="flex gap-4">
            ...
            <div>
              ...
              <p v-if="profileUser.bio" class="mt-3 text-sm text-slate-700">{{ profileUser.bio }}</p>

              <div class="mt-3 flex flex-wrap gap-4 text-sm text-slate-600">
                <span><strong class="text-slate-900">{{ profileUser.followers_count }}</strong> followers</span>
                <span><strong class="text-slate-900">{{ profileUser.following_count }}</strong> following</span>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <ul class="mt-3 space-y-3">
              <li v-for="user in followers" :key="`follower-${user.id}`">
                <Link :href="route('users.show', user.username)" class="text-sm text-slate-700 hover:underline">
                  {{ user.name }} <span class="text-slate-500">@{{ user.username }}</span>
                </Link>
              </li>
            </ul>
          </div>
